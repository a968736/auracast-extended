...
/**
 * Sanitize a browser-produced JPEG for the Jieli SoC's MJPEG decoder.
 *
 * Browsers (Chromium, Safari) inject several APP segments that the badge's
 * decoder does not handle:
 *   - APP0 (FFE0) JFIF - kept (JFIF is mandatory in MJPEG)
 *   - APP2 (FFE2) ICC color profile - STRIPPED (causes badge to render only
 *     the first few rows then bail; this was the root cause of the
 *     "first 3 pixels at top" rendering bug we hunted for two days).
 *   - APP1 (FFE1) EXIF - STRIPPED (not part of standard MJPEG-A; some Jieli
 *     decoders also choke on it).
 *   - APP13/APP14 (FFED, FFEE) Photoshop/Adobe - STRIPPED for the same reason.
 *
 * Returns a new Uint8Array with the segments removed. Costs <1ms for
 * typical 30 KB frames, no allocation in the common (no-segment) case.
 */
export function sanitizeJpegForBadge(jpeg: Uint8Array): Uint8Array {
  ...
  while (i < jpeg.length - 1) {
    // skip fill bytes
    while (i < jpeg.length && jpeg[i] === 0xff && jpeg[i + 1] === 0xff) i++
    if (jpeg[i] !== 0xff) break
    const marker = jpeg[i + 1]
    if (marker === 0xd8 || marker === 0xd9) {
      // SOI/EOI - should not appear standalone here; bail
      out.push(jpeg.subarray(i))
      break
    }
    if (marker === 0xda) {
      // SOS - copy from here to EOI verbatim (entropy-coded data follows)
      out.push(jpeg.subarray(i))
      break
    }
    ...
    // Strip non-JFIF APP segments (FFE1..FFEF) - keep only FFE0 JFIF.
    ...
    out.push(jpeg.subarray(i, segEnd))
    i = segEnd
  }
  if (!stripped) return jpeg
...
export function buildMjpgAvi(
  ...
  const frames = rawFrames.map(sanitizeJpegForBadge)
