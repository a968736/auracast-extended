...
async function writeChunkTo(target: BluetoothRemoteGATTCharacteristic, chunk: Uint8Array): Promise<void> {
  // Pass the buffer through directly. The Web Bluetooth UA copies the bytes
  // synchronously before queuing the GATT write, so a JS-side copy with
...
export async function refreshBatteryQixE87(
  ...
  if (resp.payload.length >= 2) {
    ...
    conn.batteryCharging = chargingStatus === 1
    log(`Battery via Qix: ${level}% (charging status=${chargingStatus})`)
    return { level, updatedAt, charging: chargingStatus === 1 }
  }
  log(`Unexpected Qix battery response: ${toHex(resp.payload)}`)
  return { level: null, updatedAt: '', charging: false }
}
...
/**
 * Known peripheral capability bits (speculative, based on JieLi SDK patterns).
 * Attr type 18 (peripherals_support) is a bitmask indicating hardware sensors.
 */
export const PERIPHERAL_FLAGS: Record<number, string> = {
  0: 'Heart rate sensor',
  1: 'Blood oxygen (SpO2)',
  2: 'Accelerometer',
  3: 'Step counter',
...
export function decodePeripherals(bitmask: number): string[] {
  const result: string[] = []
  for (const [bit, label] of Object.entries(PERIPHERAL_FLAGS)) {
    if (bitmask & (1 << Number(bit))) result.push(label)
  }
  return result
}
...
export async function refreshBatteryE87(conn: E87Connection, log: (msg: string) => void): Promise<E87BatteryReading> {
      ...
      try {
        ...
        if (battAttr && battAttr.decoded?.endsWith('%')) {
          ...
          if (Number.isFinite(n) && n > 0) {
            ...
            return { level: n, updatedAt: conn.batteryUpdatedAt, charging: false }
          }
        }
      } catch { /* ignore */ }
      return { level: null, updatedAt: '', charging: false }
  ...
  return { level, updatedAt, charging: false }
}
...
export async function browseFilesE87(
  ...
  await ensureE87Auth(conn, log)
  drainStaleFrames(conn.notificationQueue, log)
  const browseQueue: Uint8Array[] = []
  const mirrorBrowseNotification = (event: Event) => {
    const target = event.target as BluetoothRemoteGATTCharacteristic
    const value = target.value
    if (!value) return
    browseQueue.push(new Uint8Array(value.buffer.slice(value.byteOffset, value.byteOffset + value.byteLength)))
    if (browseQueue.length > 100) browseQueue.shift()
  }

  for (const c of conn.notifyChars) c.addEventListener('characteristicvaluechanged', mirrorBrowseNotification)
  try {
    ...
    for (const devHandler of handlersToTry) {
      drainStaleFrames(conn.notificationQueue, log)
      browseQueue.length = 0
      try {
        const ack = await sendFileBrowseRequest(conn, browseQueue, log, type, readNum, startIndex, devHandler, clusters)
      const status = ack.body[0] ?? 0xff
      const statusName = RCSP_STATUS_NAMES[status] ?? `0x${status.toString(16)}`
      ...
      const data = await waitForNotificationFrame(
        browseQueue,
        (f) => f.flag === 0x80 && f.cmd === 0x01,
        3000,
        'file browse data packet',
        log,
      )
      log(`FileBrowse[${DEV_HANDLER_NAMES[devHandler] ?? devHandler}]: data body(${data.body.length})=${toHex(data.body.slice(0, Math.min(120, data.body.length)))}`)

      const payload = data.body.length > 2 ? data.body.slice(2) : data.body
      if (payload.length === 0) {
    ...
    for (const c of conn.notifyChars) c.removeEventListener('characteristicvaluechanged', mirrorBrowseNotification)
  }
}
...
function randomTempName(): string {
  // ponytail: match official E01 app metadata names: 8 lowercase hex chars + ".tmp".
...
export async function writeFileE87(opts: UploadOptions): Promise<void> {
  ...
  // ponytail: upload flow must not inherit stale 0x1d window ACKs from earlier attempts.
  ...
  const abortUpload = async (seq: number, reason: string) => {
    try {
      await sendE87Frame(0xc0, 0x1c, Uint8Array.of(seq & 0xff, 0x00))
      log(`Aborted upload session: ${reason}`)
  ...
  const newestQueuedWindowAck = (initial: E87Frame): E87Frame => {
    let latest = initial
    let skipped = 0
    for (let i = 0; i < uploadNotificationQueue.length;) {
      const frame = parseE87Frame(uploadNotificationQueue[i])
      if (frame?.flag === 0x80 && frame.cmd === 0x1d && frame.body.length >= 8) {
        latest = frame
        uploadNotificationQueue.splice(i, 1)
        skipped += 1
      } else {
        i += 1
      }
    }
    if (skipped) log(`Skipped ${skipped} stale window ack(s); using the newest offset.`)
    return latest
  }

  const hasQueuedTransferResponse = (): boolean => uploadNotificationQueue.some((raw) => {
    const frame = parseE87Frame(raw)
    return Boolean(frame && ((frame.flag === 0x80 && frame.cmd === 0x1d) || frame.cmd === 0x20 || frame.cmd === 0x1c))
  })
  ...
  try {
    ...
    seqCounter += 1
    lastSeqForAbort = seqCounter
    const beginAck = await waitFrame((f) => f.cmd === 0x21, 8000, 'ack cmd 0x21')
    log(`cmd 0x21 ack: ${toHex(beginAck.body)}`)
    ...
    seqCounter += 1
    lastSeqForAbort = seqCounter
    const paramsAck = await waitFrame((f) => f.cmd === 0x27, 8000, 'ack cmd 0x27')
    log(`cmd 0x27 ack: ${toHex(paramsAck.body)}`)
    ...
    const metaBody = new Uint8Array(1 + 4 + 2 + nameBytes.length + 1)
    ...
    // ponytail: official E01 metadata is seq + size + crc + nul-terminated tmp name.
    metaBody.set(nameBytes, 7)
    metaBody[metaBody.length - 1] = 0x00
    log(`File metadata body: ${toHex(metaBody)} (${tempName})`)

    await sendE87Frame(0xc0, 0x1b, metaBody)
    const metaAck = await waitFrame((f) => f.cmd === 0x1b, 8000, 'ack cmd 0x1b')
    log(`File metadata ack: ${toHex(metaAck.body)}`)

    if (metaAck.body.length < 4) {
      await abortUpload(seqCounter, `metadata rejected (${toHex(metaAck.body)})`)
      try {
        const close = await waitFrame((f) => f.cmd === 0x1c, 5000, 'session close after metadata reject')
        log(`Metadata reject close: ${toHex(close.body)}`)
        uploadClosed = true
      } catch { log('No session close after metadata reject.') }
      throw new Error(`Badge rejected file creation (${toHex(metaAck.body)}). Clear old badge items or reboot the badge, then retry.`)
    }
    ...
    let maxOffsetDelivered = 0
    // Web Bluetooth resolves write-without-response when Windows queues the
    // packet, not when the badge has consumed it. Pace the queue for E01.
    const effectiveChunkDelayMs = Math.min(20, Math.max(0, interChunkDelayMs))
    log(`Data pacing: ${effectiveChunkDelayMs} ms per chunk.`)
    ...
    const sendChunksAt = async (offset: number, winSize: number) => {
      ...
      while (bytesSent < winSize) {
        ...
        if (chunksInWindow > 0 && hasQueuedTransferResponse()) {
          log('Device replied before the window was full; switching to its newest offset.')
          break
        }
        ...
        chunksInWindow += 1
        maxOffsetDelivered = Math.max(maxOffsetDelivered, chunkOffset + chunkLen)
        onProgress(
          maxOffsetDelivered,
          jpegBytes.length,
          Math.min(totalChunks, Math.ceil(maxOffsetDelivered / chunkSize)),
          totalChunks,
        )
        ...
        if (!isCommitChunk) await sleep(effectiveChunkDelayMs)
      }
      log(`Window done: sent ${chunksInWindow} chunks, ${formatBytes(bytesSent)} (delivered: ${formatBytes(maxOffsetDelivered)}/${formatBytes(jpegBytes.length)}, transmitted: ${formatBytes(totalBytesSent)})`)
    }
    ...
    const handleCompletion = async (frame: E87Frame): Promise<void> => {
      ...
      if (statusByte !== 0x00) throw new Error(`Badge rejected the upload: ${statusStr}.`)
    }
    ...
    try {
      firstWinAck = await waitFrame(
        ...
        30000, 'initial window ack (windowed flow control)'
      )
    } catch {
      await abortUpload(seqCounter, 'no initial window ack')
      throw new Error(
        'Device did not send the initial window ACK within 30 s. Cannot start data transfer. ' +
...

    {
      ...
      while (!done) {
        ...
        if (currentAck && currentAck.cmd === 0x1d && currentAck.body.length >= 8) {
          currentAck = newestQueuedWindowAck(currentAck)
          ...
          const isCommitRequest = sentChunks > 0 && nextOffset === 0 && winSize <= chunkSize
          if (!isCommitRequest && nextOffset < lastAckOffset) {
            log(`Ignoring stale window offset ${nextOffset}; last confirmed ${lastAckOffset}.`)
          } else if (nextOffset >= jpegBytes.length) {
            lastAckOffset = Math.max(lastAckOffset, nextOffset)
            log(`Window offset ${nextOffset} is at/past EOF; waiting for completion.`)
          } else {
            lastAckOffset = Math.max(lastAckOffset, nextOffset)
        ...
        const frame = await waitFrame(
          ...
          30000, 'window ack, FILE_COMPLETE, or session close'
        )
    ...
    if (!uploadClosed) await abortUpload(lastSeqForAbort, 'cleanup after failed upload')
    for (const c of conn.notifyChars) {
...
/**
 * Query the badge brightness level via SysInfo light attribute (type 7).
 * Returns a numeric value (0-100) or null if unavailable.
 */
export async function getBrightnessE87(
  conn: E87Connection,
  log: (msg: string) => void,
): Promise<number | null> {
  try {
    const info = await getSysInfoE87(conn, log, { function: 0, mask: (1 << 7) })
    const lightAttr = info.attrs.find((a) => a.type === 7)
    if (lightAttr?.decoded) {
      const n = parseInt(lightAttr.decoded, 10)
      if (Number.isFinite(n)) {
        log(`Brightness query: ${n}`)
        return n
      }
    }
  } catch (err) {
    log(`Brightness query failed: ${(err as Error).message}`)
  }
  return null
}

/**
 * Set badge brightness via the Qix control characteristic.
 *
 * NOTE: The payload structure is speculative, derived from the bootstrap
 * sequence that sends `9E20 0816 0100 01` on FD02 during connection.
 * Attribute 0x16 (22) corresponds to the "light" control in the Jieli
 * Qix protocol. The last byte is the brightness level (0-100).
 * This may need adjustment after hardware testing.
 */
export async function setBrightnessE87(
  conn: E87Connection,
  level: number,
  log: (msg: string) => void,
): Promise<void> {
  ...
  // Qix frame: cmd=0x20, payload=[attrType=0x16, dataLen=0x01, 0x00, value]
  const resp = await sendQixCommandAndWait(
    conn, 0x20, Uint8Array.of(0x16, 0x01, 0x00, value), 0x20, log, 3000,
  )
  log(`Brightness set to ${value} (response: ${toHex(resp.payload)})`)
}
